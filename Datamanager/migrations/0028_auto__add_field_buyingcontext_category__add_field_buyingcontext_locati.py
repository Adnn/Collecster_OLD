...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'BuyingContext', fields ['name']
        db.delete_unique('Datamanager_buyingcontext', ['name'])

        # Adding field 'BuyingContext.category'
        db.add_column('Datamanager_buyingcontext', 'category',
                      self.gf('django.db.models.fields.CharField')(default=u'NET', max_length=3),
                      keep_default=False)

        # Adding field 'BuyingContext.location'
        db.add_column('Datamanager_buyingcontext', 'location',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['Datamanager.Location'], null=True, blank=True),
                      keep_default=False)

        # Adding field 'BuyingContext.complement'
        db.add_column('Datamanager_buyingcontext', 'complement',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=60, blank=True),
                      keep_default=False)


        # Changing field 'Buying.location'
        db.alter_column('Datamanager_buying', 'location_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['Datamanager.Location'], null=True))

    def backwards(self, orm):
        # Deleting field 'BuyingContext.category'
        db.delete_column('Datamanager_buyingcontext', 'category')

        # Deleting field 'BuyingContext.location'
        db.delete_column('Datamanager_buyingcontext', 'location_id')

        # Deleting field 'BuyingContext.complement'
        db.delete_column('Datamanager_buyingcontext', 'complement')

        # Adding unique constraint on 'BuyingContext', fields ['name']
        db.create_unique('Datamanager_buyingcontext', ['name'])


        # User chose to not deal with backwards NULL issues for 'Buying.location'
        raise RuntimeError("Cannot reverse this migration. 'Buying.location' and its values cannot be restored.")
